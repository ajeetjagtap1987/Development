...
class License extends Controller
{
    ...
    public function licenseFilter($page_no)
    {
        ...
        if ($searchData['status'] != '') {
            if ($_POST['status'] == 0) {
                $paginateData = $LicenseModel->where('license.expiry_date !=', 0000-00-00)->where('license.expiry_date <=', date('Y-m-d'));
            } else {
                $paginateData = $LicenseModel->groupStart()
    ...
    public function exportData()
    {
        ...
        if ($searchData['status'] != '') {
            if ($_POST['status'] == 0) {
                $licenseDetails->where('license.expiry_date !=', 0000-00-00)->where('license.expiry_date <=', date('Y-m-d'));
              } else {
                  $licenseDetails->groupStart()
                  ->where('license.expiry_date >', date('Y-m-d'))
                  ->orWhere('license.expiry_date', 0000-00-00)
                       ->groupEnd();
              }
    ...
    public function details($e_id)
    {
        // print_r($e_id);
        // die();
        if (!is_file(APPPATH . 'Views/licenses/details.php')) {
            throw new \CodeIgniter\Exceptions\PageNotFoundException();
        ...
        if (count($result) > 0) {
            $data['license'] = $result;
            $license_id = $CommonModel->getDataByColumnName($table_name = 'license_name', $column_name = 'id', $value = $result[0]['license_name_id']);
            $data['license_name'] = $license_id[0]['name'];

            $data['manufacturer'] = $CommonModel->getAllDataByColumnName($table_name = 'manufacturer', $column_name = 'id', $value = $data['license'][0]['manufacturer_id']);

            return view('includes/header') . view('licenses/details', $data) . view('includes/footer');
        } else {
            session()->setFlashdata("error", "Something went wrong, please try again!!");
            return redirect()->to(base_url() . '/licenses/list');
        }
