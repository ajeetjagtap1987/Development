...
class UserDetails extends Controller
{
    ...
    public function list($e_id)
    {
        ...
        if(count($result) > 0){
            ...
            $data['application'] =  $CommonModel->getSelectiveActiveData($table_name='application',$column_name='id,name');
            ...
            if(count($user_asset_data)>=$checkcategory[0]['user_assign_limit'] && count($user_asset_data)==$checkcategory[0]['user_assign_limit']){
                ...
                if(count($user_asset_assigned)>0){
                    ...
                    if($last_id){
                        ...
                        $userDetails =  $CommonModel->getSpecificDataByColumnName($table_name = 'user', $column_name = 'id', $value = $u_id, $select = 'first_name,last_name');

                        $log = $CommonModel->logCreate('ALLOT ASSET', 'Asset id -' . $checkassets[0]['asset_id'] .' '.'alloted to user'." ". $userDetails[0]['first_name'].' '.$userDetails[0]['last_name'] );
        ...
        if(count($checkdata) > 0){
            if($checkdata[0]['user_id'] == $u_id){
                if($checkdata[0]['status'] == ASSIGNED){
                    ...
                    $asset_details =  $CommonModel->getAllDataByColumnName($table_name='assets',$column_name='id',$value=$checkdata[0]['asset_id']);

                    $userDetails =  $CommonModel->getSpecificDataByColumnName($table_name = 'user', $column_name = 'id', $value = $u_id, $select = 'first_name,last_name');

                    $log = $CommonModel->logCreate('REVOKE ASSET', 'Asset id -' . $asset_details[0]['asset_id'] .' '.'revoked from user'." ". $userDetails[0]['first_name'].' '.$userDetails[0]['last_name'] );
    ...
    public function licenseGetKeys(){
        ...
        $CommonModel = new CommonModel();
        $result = $CommonModel->getSpecificDataActiveByColumnName($table_name = 'license', $column_name = 'license_name_id',$value=$software,$select='id,product_key');
            ...
            if(count($user_licence_data)>=$checklicense[0]['user_limit'] && count($user_licence_data)==$checklicense[0]['user_limit']){
                ...
                if(count($user_license_assigned)>0){
                    ...
                    if($last_id){
                        ...
                        $userDetails =  $CommonModel->getSpecificDataByColumnName($table_name = 'user', $column_name = 'id', $value = $u_id, $select = 'first_name,last_name');
                        $license_name =  $CommonModel->getDataByColumnName($table_name='license_name',$column_name='id',$value=$checklicense[0]['license_name_id']);
                        $log = $CommonModel->logCreate(
                            'ALLOT LICENSE',
                            'license name - ' . $license_name[0]['name'] . ' , product key -' . (empty($checklicense[0]['product_key']) ? 'NA' : $checklicense[0]['product_key']) . ' alloted to user'." ". $userDetails[0]['first_name'].' '.$userDetails[0]['last_name'],
                        );
                    }else{
                        $response['status']='error';
                        $response['message']='Something went wrong, please try again!!';
        ...
        if(count($checkdata) > 0){
            if($checkdata[0]['user_id'] == $u_id){
                if($checkdata[0]['status'] == ASSIGNED){
                    ...
                    $license_info =  $CommonModel->getDataByColumnName($table_name='license',$column_name='id',$value=$checkdata[0]['license_id']);
                    ...
                    $log = $CommonModel->logCreate(
                        'REVOKE LICENSE',
                        'license name - ' . $license_name[0]['name'] . ' , product key -' . (empty($license_info[0]['product_key']) ? 'NA' : $license_info[0]['product_key']) . ' revoked from user'." ". $userDetails[0]['first_name'].' '.$userDetails[0]['last_name'],
                    );
    ...
    public function userApplicationAssign($e_id)
    {
        ...
        if(count($checkdata) > 0){
                ...
                if(count($user_application_assigned)>0){
                    ...
                    $log = $CommonModel->logCreate(
                        'ALLOT APPLICATION',
                        'Application name - ' . $checkdata[0]['name'] . ' ' .'alloted to user'." ".$userDetails[0]['first_name'].' '.$userDetails[0]['last_name'],
                    );
                }
        ...
        if(count($checkdata) > 0){
            if($checkdata[0]['user_id'] == $u_id){
                if($checkdata[0]['status'] == ASSIGNED){
                    ...
                    $app_info =  $CommonModel->getAllDataByColumnName($table_name='application',$column_name='id',$value=$checkdata[0]['application_id']);
                    $userDetails =  $CommonModel->getSpecificDataByColumnName($table_name = 'user', $column_name = 'id', $value = $u_id, $select = 'first_name,last_name');

                    $log = $CommonModel->logCreate(
                        'REVOKE APPLICATION',
                        'Application name - ' . $app_info[0]['name'] . ' ' .'revoked from user'." ".$userDetails[0]['first_name'].' '.$userDetails[0]['last_name'],
                    );
                }else{
                    $response['status']='error';
